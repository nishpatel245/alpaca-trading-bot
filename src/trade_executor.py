...
def open_long(broker, symbol: str, qty: int, price: float, risk_cfg: dict, strategy_name: str) -> bool:
    """
    Places a market BUY order with bracket (stop-loss + take-profit).
...
def close_long(broker, symbol: str, strategy_name: str) -> bool:
    """
    Closes an existing long position at market price.
    Returns True on success.
    """
...
def execute_signal(
    ...
    """
    Entry point called by the bot loop.
    Decides whether to open or close based on the signal and existing position.
    """
    ...
    if signal == "BUY" and position is None:
        ...
        open_long(broker, symbol, qty, price, risk_cfg, strategy_name)

    elif signal == "SELL" and position is not None:
        close_long(broker, symbol, strategy_name)
