...
class TradingBot:
    ...
    def _scan_cycle(self) -> None:
        ...
        for symbol in self.symbols:
            try:
                self._process_symbol(symbol, equity, pos_count, pos_syms)
            except Exception as e:
                logger.error(f"Error processing {symbol}: {e}", exc_info=True)

    def _process_symbol(
        self, symbol: str, equity: float, pos_count: int, pos_syms: set
    ) -> None:
        df = get_bars(
        ...
        # Re-read pos_count considering if we just opened positions this cycle
        if not risk_manager.all_checks_pass(signal, symbol, equity, pos_count, self.cfg):
            return
